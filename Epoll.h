...
namespace swings {
    ...
    public:
        ...
        int del(int fd, HttpRequest * request, int events); // 从epoll注册表中删除描述符
        int wait(int timeout); // 等待事件发生，返回活跃描述符数量
        ...
        void setOnClsoeConnection(const CloseConnectionCallback & cb) { onCloseConnection_ = cb; } // 设置关闭连接回调函数
        void setOnRequest(const HandleRequestCallback & cb) { onRequest_ = cb; } // 设置请求连接回调函数
